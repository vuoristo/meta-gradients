...
if __name__ == '__main__':
    config = dict(
        ...
        truncation_length=tune.grid_search([23, 30]),
        meta_discount=1.0,
        logpi_cum_fn='meta_batch_size',
        e_maml_lambda=tune.grid_search([0.0, 2.0, 6.0, 10.0]),
        opt_type='sgd',
        opt_kwargs=dict(learning_rate=0.005),
    ...
    analysis = tune.run(
        trainable,
        name='on_policy_bandits:learn_lr_schedule:lt_len_30:noise_2:eta_init_22:lr_0005:500k_steps:trunc_in_23_30:sweep_elambda',
        config=config,
        verbose=1,
        resources_per_trial={
            'gpu': 0.5,
