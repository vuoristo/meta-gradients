...
def outer_loss_vanilla(
    ...
    log = Log(pi_loss, 0.0, -ent_loss, 0.0, 0.0, advantage, 0.0)
...
def outer_loss_first_state_return(
    ...
    ent_loss = -jnp.mean(entropy)
    total_loss = pi_loss + ent_coef * ent_loss + sc_coef * sampling_correction
    log = Log(pi_loss, 0.0, -ent_loss, 0.0, 0.0, rewards, 0.0, 0.0)
    return total_loss, logpi_a.sum(), log
...
def sample_trajs(
    ...
    traj = jax.tree_map(lambda *xs: jnp.moveaxis(jnp.concatenate(xs), 0, 1),
                        traj_pre, traj_post)
    return traj
...
def get_scan_fn(
    ...
    if config['inner_loop_fn'] == 'default':
        ...
        def scan_fn(prev_state, _):
            ...
            grad = jax.tree_map(lambda g1, g2: (g1 + vf_coef * g2), grad, vf_grad)
            inner_log = inner_log._replace(grad_norm=optax.global_norm(grad))
            updates, theta_opt_state = inner_optimizer.update(grad, theta_opt_state, params=theta)
            theta = optax.apply_updates(theta, updates)
            timestep = jax.tree_map(lambda x: x[:, -1], trajs)
...
def train_step_es(
    ...
    inner_loop_fn: Any,
    es_strategy: Any,
    es_param_reshaper: Any,
    fitness_shaper: Any,
) -> Array:
    def vectorized_inner_loop(*args):
        outer_loss, outputs = jax.vmap(inner_loop_fn, (0, 0, 0, 0, 0))(*args)
        fitness = outputs[4].advantage.mean(axis=(1,2,3))
        return fitness, outputs
    k1, k2 = jrandom.split(key)
    x, es_state = es_strategy.ask(k2, es_state, es_params)
    etas = es_param_reshaper.reshape(x)
    k1, k2 = jrandom.split(k1)
    keys = jrandom.split(k2, mean_rewards.shape[0])
    loss, (rewards_out, new_theta, new_mean_rewards, new_timestep, outer_log,
        inner_log) = vectorized_inner_loop(keys, theta, etas, mean_rewards, timestep)
    fitness = fitness_shaper.apply(x, loss)
    es_state = es_strategy.tell(x, fitness, es_state, es_params)
    return (es_state, new_theta, rewards_out, new_mean_rewards, new_timestep,
            outer_log, inner_log)
...
def log_status(
    ...
    if total_steps in log_steps:
        ...
        monitor_log = jax.device_get(dict(
            iteration=iteration,
    ...
    if total_steps in eval_steps:
        ...
        es_state = jax.device_get(es_state)
        mean_rewards = jax.device_get(mean_rewards)
        ts = jax.device_get(ts)
        with open(path, 'wb') as checkpoint_file:
            pickle.dump((theta, es_state, mean_rewards, ts), checkpoint_file)
...
if __name__ == '__main__':
    ...
    if args.mode == 'ray':
        analysis = tune.run(
            trainable,
            name='1123:simple_maze:test_single_lifetime:3',
            config=config,
            resources_per_trial={
                'cpu': 3.0,
            },
