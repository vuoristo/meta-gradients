...
from algorithms.off_policy_bandits_es import trainable
...
if __name__ == '__main__':
    ...
    config = dict(
        ...
        truncation_length=tune.grid_search([2, 21, 41, 61, 80]),
        meta_discount=1.0,
        logpi_cum_fn='meta_batch_size',
        e_maml_lambda=tune.grid_search([0.0]),
        opt_type='sgd',
    ...
    analysis = tune.run(
        trainable,
        name='on_policy_bandits:learn_lr_schedule:lt_len_80:noise_2:es:range_05_25:index_cutoff_10',
        config=config,
        verbose=1,
        resources_per_trial={
            'gpu': 1.0,
