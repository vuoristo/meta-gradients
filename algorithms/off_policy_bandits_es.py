...
import pandas as pd
import numpy as np
from evosax import PersistentES, OpenES, FitnessShaper
...
class ES(OpenES):
    ...
    def initialize_strategy(
        ...
        es_state = {
            ...
            "prev_grad": jnp.zeros_like(params["eta"]),
        }
...
def append_to_buffer(
    ...
    buffer = jax.tree_map(appender, buffer, appendix)
    return buffer
...
def compute_meta_gradient(
    ...
    inner_loop_fn = functools.partial(
        ...
        config=config)
    grad_fn = jax.grad(inner_loop_fn, has_aux=True, argnums=2)
    g, aux = grad_fn(rngkey, theta, eta, mus)
    return g, aux
...
def compute_meta_gradient_es(
    ...
    def vectorized_inner_loop(*args):
        _, aux = jax.vmap(inner_loop_fn, (0, None, 0, None))(*args)
        fitness = aux[1].reward_tp1.sum(axis=1) / denom
        return fitness, aux
    k1, k2 = jrandom.split(rngkey)
    es_params = es_params.copy()
    es_params.update({"eta": eta})
    es_state = es_strategy.initialize(k2, es_params)
    etas, es_state = es_strategy.ask(k2, es_state, es_params)
    k1, k2 = jrandom.split(k1)
    keys = jrandom.split(k2, config['popsize'])
    raw_fitness, buffers = vectorized_inner_loop(keys, theta, etas, mus)
    fitness = fitness_shaper.apply(etas, raw_fitness)
    es_state = es_strategy.tell(etas, fitness, es_state, es_params)
    return es_state["prev_grad"], buffers
...
def sample_gradients(
    ...
    update_with_es = config['update_with_es']
    ...
    compute_meta_gradient_es_ = jax.jit(jax.vmap(
        functools.partial(
            compute_meta_gradient_es,
    ...
    for iteration in range(config['iterations']):
        ...
        es_g, aux = compute_meta_gradient_es_(keys[1:], theta, eta, mus, es_params)
        buffer = jax.device_get(jax.tree_map(jnp.mean, aux))[0]
        mean_es_g = -jax.device_get(es_g.mean(axis=0))
        logdict['eta_es_g0'].append(mean_es_g[0])
        logdict['eta_es_g1'].append(mean_es_g[1])
        var_es_g = jax.device_get(es_g.var(axis=0))
        logdict['eta_es_g0_var'].append(var_es_g[0])
        logdict['eta_es_g1_var'].append(var_es_g[1])
        logdict['iteration'].append(iteration)
        logdict['returns'].append(buffer.reward_tp1)
        if update_with_es:
            updates, opt_state = optimizer.update(es_g.mean(axis=0), opt_state)
...
if __name__ == '__main__':
    ...
    if args.mode == 'ray':
        analysis = tune.run(
            trainable,
            name='0727:divide_by_lifetime_len',
            config=config,
            verbose=1,
            resources_per_trial={
                'gpu': 0.3,
